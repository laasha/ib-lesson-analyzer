...
## ✉️ Email Version
[Write a human, warm, and collegial coaching letter (narrative letter) to the teacher summarizing the observation results.
Do not use timestamps. The letter should be written in a natural, professional, and supportive tone, like a real email.
Format:
Dear [Name or "colleague"],
[Introduction and positive observations]
[Discussion of growth areas softly and strategically]
...
## ✉️ მეილის ვერსია
[აქ დაწერე ადამიანური, თბილი და კოლეგიალური ქოუჩინგ-წერილი (narrative letter) მასწავლებლისთვის, რომელიც შეაჯამებს დაკვირვების შედეგებს.
არ გამოიყენო თაიმსტემპები (დროები). წერილი უნდა იყოს დაწერილი ბუნებრივი, პროფესიონალური და მხარდამჭერი ტონით, როგორც ნამდვილი წერილი.
ფორმატი: 
მოგესალმებით [სახელი ან "კოლეგავ"],
[შესავალი და პოზიტიური დაკვირვება]
[განვითარების არეალზე საუბარი რბილად და სტრატეგიულად]
...
`;
    ...
    let response;
    let errorDetails = "";
    // On Vercel, we can try gemini-2.5-flash and gemini-1.5-flash as well.
    ...
    return res.status(500).json({ error: error.message || "სერვერის შეცდომა ანალიზისას" });
  }
}
